    ...
    sLog.outString("Loading Player Create XP Data...");
    {
        ...
        do
        {
            ...
            if (current_level >= sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    sLog.outDetail("Unused (> MaxPlayerLevel in PhantomCore.conf) level %u in `player_xp_for_levels` table, ignoring.",current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 item_id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, Player* pl, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
    ...
    if (vItems->FindItemCostPair(item_id,ExtendedCost))
    {
	    ...
	    sLog.outErrorDb("Table `(game_event_)npc_vendor` has duplicate items %u (with extended cost %u) for vendor (Entry: %u), ignoring", item_id, ExtendedCost, vendor_entry);
        return false;
    }
