    ...
    *data << uint32(StatusID);                              // status
    switch(StatusID)
    {
        ...
        case STATUS_IN_PROGRESS:                            // status_in_progress
            ...
            *data << uint8(uiframe);                    // unk, possibly 0 == preparation phase, 1 == battle
            break;
        default:
...
BattleGroundQueueTypeId BattleGroundMgr::BGQueueTypeId(BattleGroundTypeId bgTypeId, uint8 arenaType)
{
    switch(bgTypeId)
    {
        ...
        case BATTLEGROUND_RV:
            ...
            {
                case ARENA_TYPE_2v2:
				if (sWorld.getConfig(CONFIG_ARENA_2v2_TEAM_ENABLE) == 1)
                    return BATTLEGROUND_QUEUE_2v2;
				else
					return BATTLEGROUND_QUEUE_NONE;
                case ARENA_TYPE_3v3:
				if (sWorld.getConfig(CONFIG_ARENA_3v3_TEAM_ENABLE) == 1)
                    return BATTLEGROUND_QUEUE_3v3;
				else
					return BATTLEGROUND_QUEUE_NONE;
                case ARENA_TYPE_5v5:
				if (sWorld.getConfig(CONFIG_ARENA_5v5_TEAM_ENABLE) == 1)
