...
namespace VMAP
{
    ...
    */
    ...
    {
        bool result = false;
        if(getNSubModel() == pMc2.getNSubModel() && getAABoxBounds() == pMc2.getAABoxBounds())
        {
            if(getNNodes() == pMc2.getNNodes() && getNTriangles() == pMc2.getNTriangles() && getBasePosition() == pMc2.getBasePosition())
            {
                result = true;
                for(unsigned int i=0; i<pMc2.getNSubModel(); ++i)
                {
                    SubModel sm1 = pMc2.getSubModel(i);
                    SubModel sm2 = getSubModel(i);
                    result = (sm1 == sm2);
                    if(!result) break;
                }
                if(result)
    ...
    */
    ...
    bool ModelContainer::readBounds(const char *filename)
    {
        bool result = false;
        ...
        if(rf)
        {
            ...
            if(result && fread(NULL,8,1,rf) != 1) result = false;
            if(result && fread(NULL,sizeof(unsigned int),1,rf) != 1) result = false;
            //POS
            if(result && fread(NULL,4,1,rf) != 1) result = false;
            if(result && fread(NULL,4,1,rf) != 1) result = false;
            if(result && fread(NULL,sizeof(float),3,rf) != 3) result = false;
